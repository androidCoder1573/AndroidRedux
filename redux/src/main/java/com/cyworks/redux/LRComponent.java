...
public abstract class LRComponent<S extends BaseComponentState> extends BaseComponent<S> {
     ...
     */
    public LRComponent(boolean lazyBindUI) {
        super(lazyBindUI);
     ...
     * @return 是否需要处理可见性变化
     */
    private boolean visibilityChanged(List<String> propKeys) {
        boolean show = mContext.getState().isShowUI.value();
    ...
    private boolean needHandleOrientation(List<String> propKeys) {
        String orientationKey = "current_orientation";
